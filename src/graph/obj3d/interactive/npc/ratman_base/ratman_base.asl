ON INIT {
 SET §teleportmode 0
 SET §teleporttime 0
 SET §stolen_gold 0		// the amount of gold stealed to the player (by the ratman)
 SET £telmarker1 "NONE"
 SET £telmarker2 "NONE"
 SET £telmarker3 "NONE"
 SET £telmarker4 "NONE"
...
//#
 ...
 SET_NPC_STAT tohit 80
 SET_NPC_STAT aimtime 300
 SET_NPC_STAT life 25
 SET_NPC_STAT backstabskill 70
 SET_XP_VALUE 400
 SET_MATERIAL FLESH
 SET_ARMOR_MATERIAL LEATHER
 SET_STEP_MATERIAL Foot_stealth
 ...
 SET §tactic 1                  //0 = normal    1 = sneak   2 = rabit  3 = caster
...
//#
 ...
 INC §scale #TMP
 SETSCALE §scale
 SETWEAPON -h Dagger_assassin
 LOADANIM WALK_SNEAK                 "ratman_walk"    //*****************  NEW
 loadanim DAGGER_READY_PART_1        "ratman_ready_knife_start"
 loadanim DAGGER_READY_PART_2        "ratman_ready_knife_end"
 loadanim DAGGER_UNREADY_PART_1      "ratman_unready_knife_start"
 loadanim DAGGER_UNREADY_PART_2      "ratman_unready_knife_end"
 loadanim DAGGER_WAIT                "ratman_fight_wait_toponly"
 loadanim DAGGER_STRIKE_LEFT_START   "ratman_fight_attack_left_start"
 loadanim DAGGER_STRIKE_LEFT_CYCLE   "ratman_fight_attack_left_cycle"
 loadanim DAGGER_STRIKE_LEFT         "ratman_fight_attack_left_strike"
 loadanim DAGGER_STRIKE_RIGHT_START  "ratman_fight_attack_right_start"
 loadanim DAGGER_STRIKE_RIGHT_CYCLE  "ratman_fight_attack_right_cycle"
 loadanim DAGGER_STRIKE_RIGHT        "ratman_fight_attack_left_strike"
 loadanim DAGGER_STRIKE_TOP_START    "ratman_fight_attack_left_start"
 loadanim DAGGER_STRIKE_TOP_CYCLE    "ratman_fight_attack_left_cycle"
 loadanim DAGGER_STRIKE_TOP          "ratman_fight_attack_right_strike"
 loadanim DAGGER_STRIKE_BOTTOM_START "ratman_fight_attack_right_start"
 loadanim DAGGER_STRIKE_BOTTOM_CYCLE "ratman_fight_attack_right_cycle"
 loadanim DAGGER_STRIKE_BOTTOM       "ratman_fight_attack_right_strike"
 LOADANIM GRUNT                      "ratman_fight_grunt"
 LOADANIM WALK                       "ratman_run"
 LOADANIM RUN                        "ratman_run_fast"
 LOADANIM WAIT                       "ratman_wait"
 LOADANIM HIT                        "ratman_receive_damage"
 LOADANIM HIT_SHORT                  "ratman_hit_short"
 LOADANIM DIE                        "ratman_death"
 loadanim FIGHT_WALK_FORWARD         "ratman_fight_walk"
 loadanim FIGHT_WALK_BACKWARD        "ratman_fight_walk_back"
 loadanim FIGHT_STRAFE_RIGHT         "ratman_strafe_right"
 loadanim FIGHT_STRAFE_LEFT          "ratman_strafe_left"
 loadanim FIGHT_WAIT                 "ratman_fight_wait"
// LOADANIM FIGHT_DIST                 "ratman_suriken"

 LOADANIM ACTION1                    "ratman_appears"
 LOADANIM ACTION2                    "ratman_misc_jumpback_intro"
 LOADANIM ACTION3				 "ratman_fight_defend_left"
 LOADANIM ACTION4				 "ratman_fight_defend_right"
 LOADANIM ACTION5				 "ratman_suriken"

 SET £hail [ratman_threat]
 SET £thief []
 SET £strike []
 SET £victory []
 SET £whogoesthere []
 ...
 SET £back2guard []
 SET £misc []
 SET £threat [ratman_threat]
 SET £search []
 SET £youmad []
 SET £dying [ratman_die]
 SET £ouch [ratman_ouch]
 SET £ouch_medium [ratman_ouchM]
 SET £ouch_strong [ratman_ouchS]
 SET £help []
 SET £comeback []
 SET £justyouwait []
...
// TELEPORT SPECIAL ATTACK ------------------------------
ON TELEPORTED
{
	HEROSAY -d "tel"
 IF ( §teleportmode == 666 )
 {
  herosay -d "teleported behind player"
  INVULNERABILITY OFF
  TELEPORT BEHIND
  SET §teleportmode 0
  SET §fighting_mode 0
  SET §next_teleport ^GAMESECONDS
  INC §next_teleport 10
  GOTO ATTACK_PLAYER
//  TIMERshuriken 1 1 GOTO THROWSHURIKEN
  ACCEPT
 }
	HEROSAY -d "tel0"
 // teleport markers
 ...
 { // invisibility spell
	HEROSAY -d "tel1"
  TIMERTeleport 1 ~§teleporttime~ SENDEVENT TELEPORTED SELF ""
  MUL §teleporttime 1000
  SPELLCAST -dsmf ~§teleporttime~ 1 INVISIBILITY SELF
  ACCEPT
 }
 IF ( §teleportmode == 3 )
 { // end of anim -> teleport
	HEROSAY -d "tel2"
  OBJECT_HIDE SELF ON
  IF ( §telnum == 0 )
  {
...
>>TELEPORT_END
	HEROSAY -d "tel4"
 // end of teleport time -> back to fight !
 HEROSAY -d "i'm back"
 TIMERTeleport OFF
 SET §teleportmode 0
 SET §fighting_mode 0
...
>>THROWSHURIKEN
//[FLURE] le ratman lance (pitete) un objet
SPELLCAST -k INVISIBILITY
...
RANDOM 80
{
 IF (^DIST_PLAYER < 300)
 {
  HEROSAY -d "too near to throw"
  ACCEPT
 }
 HEROSAY -d "throw"
 PLAY -o "throw_fishing_line"
//  PLAY -p "whoosh06"
//	GOSUB SAVE_BEHAVIOR
// BEHAVIOR FRIENDLY
 SETTARGET PLAYER
 PLAYANIM -e ACTION5 GOTO THROWEND
 ACCEPT
 >>THROWEND
 RANDOM 40
 {
...
>>SPECIAL_ATTACK
 IF ( §teleportmode == 0 )
 {
  RANDOM 20
  {
   IF ( ^GAMESECONDS < §next_teleport ) ACCEPT
   BEHAVIOR FRIENDLY
   SETTARGET PLAYER
   INVULNERABILITY ON
   PLAYANIM -ne ACTION2 SENDEVENT TELEPORTED SELF ""
	HEROSAY -d "t0"
   IF ( §telnum == 0 ) GOTO BEHIND_PLAYER
	HEROSAY -d "t1"
   RANDOM 50
   {
 >>BEHIND_PLAYER
	HEROSAY -d "t2"
	  SET §teleporttime ~^RND_10~
        INC §teleporttime 2
	  MUL §teleporttime 1000
	  SPELLCAST -dsmf ~§teleporttime~ 1 INVISIBILITY SELF
    SET §teleportmode 666
    ACCEPT
   }
   ELSE
   {
    ...
    SET §teleporttime ~^RND_10~
    INC §teleporttime 2
    TIMERInvisibility  1 1 SENDEVENT TELEPORTED SELF ""
    ACCEPT
   }
   ACCEPT
  }
  ELSE
  {
   RANDOM 20 
   {
HEROSAY -d "RAT DEFENSE"
     SET §teleportmode 1
     INVULNERABILITY ON
     BEHAVIOR FRIENDLY
     SETTARGET PLAYER
     HALO -oc 0 0 1
     PLAYANIM -ne ACTION3 GOTO END_DEFENSE
     ACCEPT
   }
  }
 }
